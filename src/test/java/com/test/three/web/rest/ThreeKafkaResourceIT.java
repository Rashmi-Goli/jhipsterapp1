...
import com.test.three.config.KafkaProperties;
...
class ThreeKafkaResourceIT {
    ...
    void setup() {
        ...
        ThreeKafkaResource kafkaResource = new ThreeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/three-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/three-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
